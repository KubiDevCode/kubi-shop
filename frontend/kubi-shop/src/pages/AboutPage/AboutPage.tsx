...
import { SectionLabel } from "@/widgets/SectionLabel/SectionLabel";
import { Carousel } from "@/shared/UI/Carousel/Carousel";




const AboutQualityComponent = ({title, description, SVGComponent}: {
  title: string,
  description: string,
  ...
  return (
    ...
    <div className="flex-1 inline-flex flex-col justify-start items-start gap-1">
        <div className="self-stretch justify-start text-neutral-800 text-xl font-extralight font-['Outfit'] uppercase tracking-wider">{title}</div>
        <div className="self-stretch justify-start text-neutral-800 text-xl font-extralight font-['Outfit'] tracking-tight">{description}</div>
    </div>
</div>
  )
}

const RewiewComponent = ({authorName, contnent, amountOfStars = 5}: {
authorName: string,
contnent: string,
amountOfStars?: number
  ...
  return (
    <div className="self-stretch p-10 bg-white rounded-[10px] outline outline-1 outline-offset-[-1px] outline-neutral-200 inline-flex flex-col justify-center items-start gap-7">
    <div className="self-stretch justify-start text-neutral-800 text-xl font-extralight font-['Outfit'] tracking-tight">“{contnent}”</div>
    <div className="size- flex flex-col justify-start items-start gap-2">
        <div className="size- inline-flex justify-start items-start gap-1.5">
            {Array.from({length: amountOfStars}).map(() => <RewiewStarSVG/> )}
        </div>
        <div className="justify-start text-neutral-800 text-xl font-normal font-['Outfit'] capitalize tracking-tight">{authorName}</div>
    </div>
</div>
  )
}
    ...
    return (
     ...
     <PageHero title="About"/>
      ...
      <section className="section">
        <Container className="grid grid-cols-4 gap-[20px]">
      <AboutQualityComponent 
      title="Free delivery" 
      description="Consectetur adipi elit lorem ipsum dolor sit amet." 
      SVGComponent={CartSVG}/>
      <AboutQualityComponent title="Quality guarantee" description="Dolor sit amet orem ipsu mcons ectetur adipi elit." SVGComponent={CupSVG}/>
      <AboutQualityComponent title="Daily offers" description="Amet consectetur adipi elit loreme ipsum dolor sit." SVGComponent={PriceTagSVG}/>
      <AboutQualityComponent title="100% secure payment" description="Rem Lopsum dolor sit amet, consectetur adipi elit." SVGComponent={ChatSVG}/>
        </Container>
      </section>
      <section className="section">
        <Container>
        <SectionLabel title="Customers reviews"/>
        <Carousel slidesView={3} gap={20} sliderButton="outside">
      <RewiewComponent 
