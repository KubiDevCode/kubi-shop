...
export const Carousel = (props: CarouselProps) => {
  const { className, slideClassName, children, slidesView = 1, gap = 0, options, sliderButton = 'outside' } = props
    ...
    <section className={classNames('relative', className)}>
      {sliderButton === 'outside' && (
        <Button
          ...
          def={false}
          className={classNames(buttonClassName, 'absolute -left-20 top-1/2 z-10 -translate-y-1/2')}
        >
          <SliderLeft />
        </Button>
      )}
      ...
      {sliderButton === 'outside' && (
        <Button
          ...
          def={false}
          className={classNames(buttonClassName, 'absolute -right-20 top-1/2 z-10 -translate-y-1/2')}
        >
          <SliderRight />
        </Button>
      )}
      ...
      {sliderButton === 'inside' && (
        <>
          <Button
            onClick={() => emblaApi?.scrollPrev()}
            def={false}
            className={classNames(buttonClassName, 'absolute left-4 top-1/2 z-10 -translate-y-1/2')}
          >
            <SliderLeft />
          </Button>
          ...
          <Button
            onClick={() => emblaApi?.scrollNext()}
            def={false}
            className={classNames(buttonClassName, 'absolute right-4 top-1/2 z-10 -translate-y-1/2')}
          >
            <SliderRight />
          </Button>
        </>
